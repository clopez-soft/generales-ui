  ...
  const [mayorResults, { loading, error }] = useLazyQuery(MAYOR_RESULT, {
    ...
    onCompleted: (datos) => {
      ...
      for (const id of uniqueIds) {
        const rows = newPosition.filter((item) => item.politic_party_id === id);
        if (rows.length > 0) {
          const newRow: MayorResumen = {
            politic_party_id: rows[0].politic_party_id || "",
            politic_party_code: rows[0].politic_party_code || "",
            politic_party_image: rows[0].politic_party_image || "",
              ...
              <Grid item xs={12} md={4}>
                ...
                <List sx={{ width: "100%", bgcolor: "background.paper" }}>
                  {mayorPosition?.map((item, index) => {
                    return (
                      <ListItem
                        key={`ListItem-${index}-${item.politic_party_id}`}
                      >
                        <ListItemAvatar>
                          <Avatar
                            src={item.politic_party_image}
                            alt={item.politic_party_code}
                            variant="square"
                            sx={{
                              height: "64px",
                              width: "128px",
                        ...
                        <ListItemText
                          primary={`Partido - ${item.politic_party_code}`}
