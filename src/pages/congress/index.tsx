  ...
  const { loading, error } = useQuery(CONGRESS_RESULT, {
    ...
    onCompleted: (datos) => {
      ...
      const newPositions: QuotientParty[] = positions.map((item: any) => {
        ...
        return {
          politic_party_id: item.politic_party_id || "",
          politic_party_code: party?.politic_party_code || "",
          politic_party_image: party?.politic_party_image || "",
          marks: +item.marks || 0,
  ...
  useEffect(() => {
    ...
    for (const party of quotientParty) {
      ...
      const candidateParty = candiatesList.filter(
        (c) => c.politic_party_id === party.politic_party_id
      );
    ...
    setCandiatesElectos(sortElectos);
    setCandiatesListSort(sortNewList);
